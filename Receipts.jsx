...
export default function Receipts() {
  ...
  async function load() {
    try {
      const [r, c] = await Promise.all([receiptsApi.list(), customersApi.list()]);
  ...
  return (
    <div>
      <SectionHeader
        ...
        right={<>
          <Btn onClick={() => exportCSV('receipts', [
            ['Date', 'Bill no', 'Name', 'Model', 'Sold', 'Paid', 'Balance', 'Mode', 'Bank', 'UTR', 'Branch'],
            ...filtered.map(r => [r.date, r.billNo, r.name, r.model, r.soldPrice, r.amtPaid, r.balance, r.mode, r.bank, r.utr, r.branch])
          ])}>Export CSV</Btn>
            ...
            <TD>
              <div style={{ display: 'flex', gap: 3 }}>
                ...
                {role === 'admin' && <>
                  ...
                  <Btn variant="d" style={{ fontSize: 10, padding: '2px 7px' }} onClick={async () => { await receiptsApi.remove(r.id); load(); notify('Deleted.'); }}>Del</Btn>
                </>}
              </div>
            </TD>
          </TR>
...
function AddReceiptModal({ onClose, onSaved, receipts, customers, role, branch }) {
  ...
  useEffect(() => {
    setBillNo(getNextBillNo(receipts, custBranch));
  }, [custBranch]);
  ...
  function onCustSearch(q) {
    ...
    setSuggestions(customers.filter(c => {
      ...
      if (c.name?.toLowerCase().includes(q.toLowerCase()) || c.phone?.includes(q)) { seen.add(c.id); return true; }
      return false;
    }).slice(0, 6));
  }

  function selectCust(c) {
    setSelCust(c); setMode('existing');
    setCustSearch(c.name + ' (' + c.phone + ')'); setSuggestions([]);
    // find receipts for this customer
    const custReceipts = receipts.filter(r => (r.custId === c.id || r.billNo === c.billNo) && !r.isPayment);
    if (custReceipts.length) setSelReceiptId(custReceipts[0].id);
  }
  ...
  async function save() {
    ...
    try {
      if (mode === 'existing') {
        ...
        if (role === 'branch_manager' && exDate < today()) { notify('⚠️ Branch managers cannot backdate receipts'); setSaving(false); return; }
        ...
        await receiptsApi.create({
          date: exDate, billNo: selReceipt.billNo, custId: selCust.id,
          name: selCust.name, village: selCust.village, mandal: selCust.mandal,
          model: selReceipt.model, models: selReceipt.models,
          soldPrice: selReceipt.soldPrice, amtPaid: parseFloat(exAmount),
          balance: newBal, mode: exMode, bank: exMode === 'Bank transfer' ? exBank : '',
          utr: exMode === 'Bank transfer' ? exUtr : '', branch: selCust.branch, isPayment: true,
        });
        ...
        if (role === 'branch_manager' && date < today()) { notify('⚠️ Branch managers cannot backdate receipts'); setSaving(false); return; }
        const modelSummary = modelRows.map(r => r.model + (r.qty > 1 ? ' x' + r.qty : '')).join(', ');
        // Create customer
        const custId = Date.now().toString();
        await customersApi.create({
          id: custId, date, billNo, name, phone, village, mandal,
          model: modelSummary, soldPrice: grand, totalPaid: parseFloat(paid) || 0,
          branch: custBranch, status: 'Pending', deliveredDate: null, models: modelRows,
        });
        await receiptsApi.create({
          date, billNo, custId, name, village, mandal,
          model: modelSummary, models: modelRows, soldPrice: grand,
          amtPaid: parseFloat(paid) || 0, balance, mode: payMode,
          bank: payMode === 'Bank transfer' ? bank : '',
          utr: payMode === 'Bank transfer' ? utr : '',
          branch: custBranch, isPayment: false,
        });
  ...
  return (
    <Modal open title={mode === 'existing' ? 'Record payment' : 'Add receipt'} onClose={onClose} width={480}>
      <Field label="Search existing customer (leave blank for new)">
        <div style={{ position: 'relative' }}>
          ...
          {suggestions.length > 0 && (
            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#fff', border: '1px solid #d0cfc8', borderRadius: 7, zIndex: 100, maxHeight: 160, overflowY: 'auto', marginTop: 2, boxShadow: '0 4px 16px rgba(0,0,0,.1)' }}>
              {suggestions.map(c => {
                const bal = (c.soldPrice || 0) - (c.totalPaid || 0);
                return (
                  <div key={c.id} onMouseDown={() => selectCust(c)} style={{ padding: '8px 10px', cursor: 'pointer', borderBottom: '1px solid #f0efec' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ fontWeight: 600, fontSize: 12 }}>{c.name}</span>
                      <span style={{ fontSize: 11, color: bal > 0 ? '#A32D2D' : '#27500A', fontWeight: 600 }}>{bal > 0 ? '₹' + fmt(bal) + ' due' : 'Fully paid'}</span>
                    </div>
                    <div style={{ fontSize: 10, color: '#706f6b' }}>{c.phone} · {c.billNo} · {c.branch}</div>
                  </div>
                );
              })}
            </div>
      ...
      {mode === 'existing' && selCust && (
        <>
          <div style={{ background: '#EEEDFE', borderRadius: 8, padding: '9px 12px', marginBottom: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontWeight: 600, fontSize: 12, color: '#3C3489' }}>✓ Existing customer</div>
              <div style={{ fontSize: 11, color: '#706f6b' }}>{selCust.phone} · {selCust.village} · {selCust.branch}</div>
            </div>
          ...
          {selReceipt && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 12 }}>
              {[
                { label: 'Sold price', value: '₹' + fmt(selReceipt.soldPrice), bg: '#F0EFFD', color: '#534AB7' },
                { label: 'Already paid', value: '₹' + fmt(selReceipt.amtPaid), bg: '#EAF3DE', color: '#27500A' },
                { label: 'Balance due', value: '₹' + fmt(selReceipt.balance), bg: '#FCEBEB', color: '#A32D2D' },
              ].map(s => (
                <div key={s.label} style={{ background: s.bg, borderRadius: 7, padding: 10, textAlign: 'center' }}>
          ...
          {exMode === 'Bank transfer' && <>
            <Field label="Bank"><Select value={exBank} onChange={e => setExBank(e.target.value)} options={[{ value: '', label: 'Select bank' }, 'Canara', 'TEW', 'STEW', 'Agros']} /></Field>
            <Field label="UTR ID"><Input value={exUtr} onChange={e => setExUtr(e.target.value)} placeholder="UTR reference" /></Field>
          </>}
        </>
      )}
      ...
      {mode === 'new' && (
        <>
          <Grid2>
            ...
            <Field label="Bill no (auto)"><Input value={billNo} onChange={e => setBillNo(e.target.value)} readonly /></Field>
          </Grid2>
          <Grid2>
            <Field label="Name"><Input value={name} onChange={e => setName(e.target.value)} placeholder="Customer name" /></Field>
            <Field label="Phone"><Input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Phone number" /></Field>
          </Grid2>
          <Grid2>
          ...
          <Field label="Branch">
            {role === 'admin'
              ? <Select value={custBranch} onChange={e => setCustBranch(e.target.value)} options={BRANCHES} />
              : <Input value={custBranch} readonly />}
          </Field>
          <Field label={<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>Models <Btn variant="s" style={{ fontSize: 10, padding: '2px 7px' }} onClick={() => setModelRows([...modelRows, { model: 'Model A', qty: 1, price: '' }])}>+ Add model</Btn></div>}>
            {modelRows.map((row, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 60px 100px 26px', gap: 6, alignItems: 'center', marginBottom: 6, padding: '7px 9px', background: '#f7f7f5', borderRadius: 7, border: '1px solid #e3e2dc' }}>
          ...
          <div style={{ background: '#f7f7f5', borderRadius: 8, padding: '10px 13px', marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 6, alignItems: 'center' }}>
              <span style={{ color: '#706f6b' }}>Amount paid (₹)</span>
              <input type="number" value={paid} onChange={e => setPaid(e.target.value)} placeholder="0" style={{ width: 130, padding: '4px 8px', fontSize: 12, border: '1px solid #d0cfc8', borderRadius: 6, textAlign: 'right' }} />
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, borderTop: '1px solid #e3e2dc', paddingTop: 6 }}>
              <span style={{ fontWeight: 500, color: '#706f6b' }}>Balance</span>
              <span style={{ fontWeight: 700, fontSize: 14, color: balance > 0 ? '#A32D2D' : '#27500A' }}>₹{fmt(balance)}</span>
            </div>
          </div>
          ...
          {payMode === 'Bank transfer' && <>
            <Field label="Bank"><Select value={bank} onChange={e => setBank(e.target.value)} options={[{ value: '', label: 'Select bank' }, 'Canara', 'TEW', 'STEW', 'Agros']} /></Field>
            <Field label="UTR ID"><Input value={utr} onChange={e => setUtr(e.target.value)} placeholder="UTR reference" /></Field>
          </>}
        </>
      )}
...
function ViewReceiptModal({ receipt: r, onClose }) {
  return (
    <Modal open title="Receipt details" onClose={onClose} width={460}>
      {[
        ['Date', r.date], ['Bill number', r.billNo], ['Customer', r.name],
        ['Village / Mandal', (r.village || '—') + ', ' + (r.mandal || '—')], ['Branch', r.branch],
      ].map(([l, v]) => (
        <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: '1px solid #f0efec', fontSize: 12 }}>
          <span style={{ color: '#706f6b' }}>{l}</span><span style={{ fontWeight: 500 }}>{v}</span>
        </div>
      ))}
      {r.models?.length > 0 && (
        <div style={{ margin: '10px 0', border: '1px solid #e3e2dc', borderRadius: 8, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 50px 80px 80px', background: '#f7f7f5', padding: '6px 10px', fontSize: 10, fontWeight: 600, color: '#706f6b', textTransform: 'uppercase' }}>
            <span>Model</span><span style={{ textAlign: 'center' }}>Qty</span><span style={{ textAlign: 'right' }}>Price</span><span style={{ textAlign: 'right' }}>Total</span>
          </div>
          {r.models.map((m, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 50px 80px 80px', padding: '7px 10px', borderTop: '1px solid #e3e2dc', fontSize: 12 }}>
              <span>{m.model}</span><span style={{ textAlign: 'center', color: '#706f6b' }}>{m.qty}</span>
              <span style={{ textAlign: 'right', color: '#706f6b' }}>₹{fmt(m.price)}</span>
              <span style={{ textAlign: 'right', fontWeight: 600 }}>₹{fmt(m.rowTotal)}</span>
            </div>
          ))}
        </div>
      )}
      {[
        ['Grand total', '₹' + fmt(r.soldPrice)],
        ['Amount paid', '₹' + fmt(r.amtPaid)],
...
function EditReceiptModal({ receipt, onClose, onSaved }) {
  ...
  async function save() {
    setSaving(true);
    try {
      await receiptsApi.update(receipt.id, { date, soldPrice: parseFloat(sold), amtPaid: parseFloat(paid), balance: bal, mode });
      notify('Updated!'); onSaved();
    } catch (e) { notify('Error: ' + e.message); }
    finally { setSaving(false); }
  }
  return (
    <Modal open title="Edit receipt" onClose={onClose}>
      <Field label="Date"><Input type="date" value={date} onChange={e => setDate(e.target.value)} /></Field>
      <Grid2>
        <Field label="Sold price"><Input type="number" value={sold} onChange={e => setSold(e.target.value)} /></Field>
        <Field label="Amount paid"><Input type="number" value={paid} onChange={e => setPaid(e.target.value)} /></Field>
      </Grid2>
