...
export default function Dashboard(){
  ...
  useEffect(()=>{dashboardApi.stats().then(setStats).catch(e=>setError(e.message)).finally(()=>setLoading(false));},[]);
  ...
  return <div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
      <div style={{background:'#fff',border:'1px solid #e3e2dc',borderRadius:8,padding:15}}>
        <div style={{fontSize:13,fontWeight:600,marginBottom:11}}>Customer status {role==='branch_manager'?'— '+branch:''}</div>
        <Stats items={[{label:'Pending',value:stats?.pendingCustomers||0,color:'#633806'},{label:'Delivered',value:stats?.deliveredCustomers||0,color:'#27500A'},{label:'Total',value:stats?.totalCustomers||0}]}/>
      </div>
      <div style={{background:'#fff',border:'1px solid #e3e2dc',borderRadius:8,padding:15}}>
