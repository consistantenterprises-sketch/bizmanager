...
const ACCESS={admin:['dashboard','receipts','deposits','expenses','cashflow','bank','stock','customers','feedback','bookings','employees','attendance','security'],branch_manager:['dashboard','receipts','deposits','expenses','stock','customers','feedback','bookings','attendance'],stock_manager:['dashboard','stock']};
const NAV=[{group:'Overview',items:[{id:'dashboard',label:'Dashboard'}]},{group:'Finance',items:[{id:'receipts',label:'Receipts'},{id:'deposits',label:'Deposits'},{id:'expenses',label:'Expenses'},{id:'cashflow',label:'Cash flow'},{id:'bank',label:'Bank statement'}]},{group:'Operations',items:[{id:'stock',label:'Stock'},{id:'customers',label:'Customers'},{id:'feedback',label:'Feedback calls'},{id:'bookings',label:'Bookings'}]},{group:'People',items:[{id:'employees',label:'Employees'},{id:'attendance',label:'Attendance'}]},{group:'System',items:[{id:'security',label:'Security audit'}]}];
...
export default function AppShell(){
  ...
  return <div style={{display:'flex',height:'100vh',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',background:'#f0efec',overflow:'hidden'}}>
    <NotifyProvider/>
    <div style={{width:195,minWidth:195,flexShrink:0,background:'#f7f7f5',borderRight:'1px solid #e3e2dc',display:'flex',flexDirection:'column',overflowY:'auto'}}>
      ...
      {NAV.map(({group,items})=><div key={group}>
        ...
        {items.map(({id,label})=>{const locked=!allowed.includes(id),active=page===id;return<div key={id} onClick={()=>!locked&&setPage(id)} style={{display:'flex',alignItems:'center',gap:8,padding:'7px 16px',fontSize:12,color:active?'#1a1915':locked?'#ccc':'#706f6b',cursor:locked?'not-allowed':'pointer',borderLeft:`2px solid ${active?'#534AB7':'transparent'}`,background:active?'#fff':'transparent',fontWeight:active?500:400,userSelect:'none'}}><div style={{width:6,height:6,borderRadius:'50%',flexShrink:0,background:active?'#534AB7':'#d0cfc8'}}/>{label}</div>;})}
      </div>)}
