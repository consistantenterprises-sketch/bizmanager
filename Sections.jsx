...
import { fmt, today, addDays, addMonths, daysDiff, BRANCHES, EXPENSE_TYPES } from '../lib/utils';
...
export function FeedbackCalls() {
  ...
  async function load() {
    try {
      ...
      setFeedbacks(f); setCustomers(c);
      // Auto-generate feedback records for delivered customers
      generateFeedback(c, f);
    } catch (e) { notify('Error: ' + e.message); } finally { setLoading(false); }
  }

  function generateFeedback(custs, existing) {
    const records = [];
    const td = today();
    custs.filter(c => c.status === 'Delivered' && c.deliveredDate).forEach(cust => {
      for (let cy = 0; cy <= 3; cy++) {
        const due = cy === 0 ? addDays(cust.deliveredDate, 10) : addMonths(cust.deliveredDate, cy * 4);
        const diff = daysDiff(td, due);
        if (diff > 7) continue;
        const key = `${cust.id}-${cy}`;
        const ex = existing.find(f => f.key === key);
  ...
  async function updateFB(key, data) {
    try { await feedbackApi.update(key, data); load(); } catch (e) { notify('Error: ' + e.message); }
  }
  ...
  return (
    <div>
      <SectionHeader
        left={<div style={{ background: '#f7f7f5', borderRadius: 8, padding: '9px 13px', fontSize: 11, color: '#706f6b' }}>📞 Appears 10 days after delivery · Repeats every 4 months · Approve (admin only) removes permanently</div>}
        right={<Btn onClick={() => exportCSV('feedback', [['Due date','Name','Phone','Model','Delivered','Attempts','Feedback','Status','Branch'], ...filtered.map(r => [r.due, r.cust?.name, r.cust?.phone, r.cust?.model, r.cust?.deliveredDate, r.attempts, r.feedback, r.status, r.cust?.branch])])}>Export CSV</Btn>}
      />
      <Stats items={[
        { label: 'Pending', value: filtered.filter(r => r.status === 'pending').length, color: '#633806' },
        { label: 'Overdue', value: filtered.filter(r => r.status === 'pending' && r.diff < 0).length, color: '#A32D2D' },
        { label: 'Booking raised', value: filtered.filter(r => r.status === 'booking').length, color: '#0C447C' },
        { label: 'Closed', value: filtered.filter(r => r.status === 'closed').length },
      ]} />
      <FilterBar>
          ...
          <TR key={r.key}>
            ...
            <TD>
              <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                {[1,2,3].map(n => <span key={n} onClick={() => r.status === 'pending' && updateFB(r.key, { attempts: r.attempts >= n ? n-1 : n })} style={{ width: 20, height: 20, borderRadius: '50%', border: `1.5px solid ${r.attempts >= n ? '#534AB7' : '#d0cfc8'}`, background: r.attempts >= n ? '#534AB7' : '#f7f7f5', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: r.attempts >= n ? '#fff' : '#a8a79f' }}>{n}</span>)}
                <span style={{ fontSize: 10, color: '#706f6b' }}>{r.attempts}/3</span>
              </div>
            </TD>
            <TD>
              {r.status === 'pending'
                ? <select value={r.feedback || ''} onChange={e => updateFB(r.key, { feedback: e.target.value })} style={{ fontSize: 11, padding: '3px 6px', border: '1px solid #d0cfc8', borderRadius: 6, background: '#fff' }}>
                    <option value="">—</option><option value="satisfied">😊 Satisfied</option><option value="dissatisfied">😞 Dissatisfied</option>
                  </select>
                : r.feedback ? <span style={{ fontSize: 11 }}>{r.feedback === 'satisfied' ? '😊 Satisfied' : '😞 Dissatisfied'}</span> : '—'}
            </TD>
            <TD><Badge label={r.status === 'pending' ? 'Pending' : r.status === 'booking' ? 'booking' : 'closed'} /></TD>
            <TD>
              <div style={{ display: 'flex', gap: 3 }}>
                {r.status === 'pending' && <>
                  <Btn variant="bk" style={{ fontSize: 10, padding: '2px 6px' }} onClick={() => setShowRaise(r)}>Booking</Btn>
                  <Btn style={{ fontSize: 10, padding: '2px 6px' }} onClick={() => updateFB(r.key, { status: 'closed' })}>Close</Btn>
                </>}
...
function RaiseBookingModal({ fb, onClose, onSaved }) {
  ...
  async function save() {
    if (!planDate) { notify('Select purchase planning date'); return; }
    setSaving(true);
    try {
      await bookingsApi.create({ date, name: fb.cust.name, phone: fb.cust.phone, village: fb.cust.village, model: fb.cust.model, planDate, branch: fb.cust.branch, status: 'pending', fromFeedback: true });
      notify('Booking raised for ' + fb.cust.name); onSaved();
    } catch (e) { notify('Error: ' + e.message); } finally { setSaving(false); }
  }
  return (
    <Modal open title="Raise booking" onClose={onClose}>
      <div style={{ background: '#EEEDFE', borderRadius: 8, padding: '9px 12px', marginBottom: 13, display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#3C3489' }}>
        <strong>{fb.cust.name}</strong><span>{fb.cust.phone}</span>
      </div>
...
export function Bookings() {
  ...
  useEffect(() => { load(); }, []);
  async function load() {
    try { setBookings(await bookingsApi.list()); } catch (e) { notify('Error: ' + e.message); } finally { setLoading(false); }
  }
  ...
  return (
    <div>
      <SectionHeader left={<span style={{ fontSize: 12, color: '#706f6b' }}>From feedback calls or added manually</span>} right={<>
        <Btn onClick={() => exportCSV('bookings', [['Date','Name','Phone','Village','Model','Plan date','Branch','Status'], ...filtered.map(b => [b.date,b.name,b.phone,b.village,b.model,b.planDate,b.branch,b.status])])}>Export CSV</Btn>
        <Btn variant="p" onClick={() => setShowAdd(true)}>+ Add booking</Btn>
      </>} />
      <Stats items={[
        { label: 'Total', value: bookings.length },
        { label: 'Pending', value: bookings.filter(b => b.status === 'pending').length, color: '#633806' },
        { label: 'Delivered', value: bookings.filter(b => b.status === 'delivered').length, color: '#27500A' },
        { label: 'From feedback', value: bookings.filter(b => b.fromFeedback).length, color: '#0C447C' },
      ]} />
      <FilterBar>
        <FInput value={search} onChange={setSearch} placeholder="Search name / phone..." />
...
function BookingModal({ booking, onClose, onSaved, branch }) {
  ...
  const [saving, setSaving] = useState(false);
  async function save() {
    if (!date || !name || !phone || !model || !planDate) { notify('Fill all required fields'); return; }
    setSaving(true);
    try {
      const data = { date, name, phone, village, model, planDate, branch: bkBranch, status: booking?.status || 'pending', fromFeedback: booking?.fromFeedback || false };
      if (booking) await bookingsApi.update(booking.id, data); else await bookingsApi.create(data);
      notify('Saved!'); onSaved();
    } catch (e) { notify('Error: ' + e.message); } finally { setSaving(false); }
  }
  return (
    <Modal open title={booking ? 'Edit booking' : 'Add booking'} onClose={onClose}>
      <Field label="Booking date"><Input type="date" value={date} onChange={e => setDate(e.target.value)} /></Field>
      ...
      <Field label="Date of purchase planning"><Input type="date" value={planDate} onChange={e => setPlanDate(e.target.value)} /></Field>
      <Field label="Branch">
        {role === 'admin' ? <Select value={bkBranch} onChange={e => setBkBranch(e.target.value)} options={BRANCHES} /> : <Input value={bkBranch} readonly />}
      </Field>
...
export function Employees() {
  ...
  useEffect(() => { load(); }, []);
  async function load() {
    try { setEmployees(await employeesApi.list()); } catch (e) { notify('Error: ' + e.message); } finally { setLoading(false); }
  }
  ...
  return (
    <div>
      <SectionHeader left={<span style={{ fontSize: 12, color: '#706f6b' }}>Employee accounts &amp; credentials — admin only</span>} right={<Btn variant="p" onClick={() => setShowAdd(true)}>+ Add employee</Btn>} />
      <FilterBar>
        <FInput value={search} onChange={setSearch} placeholder="Search name / username..." />
...
function EmployeeModal({ employee, onClose, onSaved }) {
  ...
  const [saving, setSaving] = useState(false);
  async function save() {
    if (!name || !email) { notify('Fill required fields'); return; }
    setSaving(true);
    try {
      if (employee) {
        await employeesApi.update(employee.id, { name, phone, branch: empBranch, role: empRole });
        if (empRole !== employee.role || empBranch !== employee.branch) {
          await employeesApi.setRole({ uid: employee.id, role: empRole, branch: empBranch });
        }
      } else {
        if (!password) { notify('Password required for new employee'); setSaving(false); return; }
        await employeesApi.createUser({ email, password, name, role: empRole, branch: empBranch, phone });
      }
      notify('Saved!'); onSaved();
    } catch (e) { notify('Error: ' + e.message); } finally { setSaving(false); }
  }
  return (
    <Modal open title={employee ? 'Edit employee' : 'Add employee'} onClose={onClose}>
...
export function Attendance() {
  ...
  async function load() {
    try {
      const [a, { employeesApi }] = await Promise.all([attendanceApi.list(), import('../lib/api')]);
      const emps = await employeesApi.list();
      setAttData(a);
      setStaff(emps.filter(e => e.status === 'active').map(e => ({ id: e.id, name: e.name, branch: e.branch })));
    } catch (e) { notify('Error: ' + e.message); } finally { setLoading(false); }
  }

  async function mark(staffId, date, status, empBranch) {
    try {
      await attendanceApi.mark({ staffId, date, status, branch: empBranch });
      load(); setPopup(null);
    } catch (e) { notify('Error: ' + e.message); }
  }

  const visibleStaff = staff.filter(s => {
    if (role === 'branch_manager' && s.branch !== branch) return false;
    if (branchF !== 'all' && s.branch !== branchF) return false;
    return true;
  });

  function getAtt(staffId, day) {
    const key = `${staffId}_${curY}-${String(curM + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
    return attData.find(a => a.id === key)?.status || null;
  }

  const attColors = { Present: { bg: '#EAF3DE', color: '#27500A' }, Absent: { bg: '#FCEBEB', color: '#A32D2D' }, Leave: { bg: '#FAEEDA', color: '#633806' }, 'Half day': { bg: '#E6F1FB', color: '#0C447C' } };
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 7, alignItems: 'center' }}>
          {role === 'admin' && <FSelect value={branchF} onChange={setBranchF} options={[{ value: 'all', label: 'All branches' }, ...BRANCHES.map(b => ({ value: b, label: b }))]} />}
          <Btn variant="p" onClick={() => setShowAdd(true)}>+ Add staff</Btn>
        </div>
      </div>
            ...
            <table style={{ borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr>
                  ...
                  {Array.from({ length: dim }, (_, i) => i + 1).map(d => {
                    ...
                    return <th key={d} style={{ minWidth: 24, padding: '4px 2px', textAlign: 'center', fontSize: 9, background: isSun ? '#F1EFE8' : '#f7f7f5', color: isSun ? '#888780' : '#706f6b', borderBottom: '1px solid #e3e2dc', fontWeight: 600 }}>{isToday ? <span style={{ background: '#534AB7', color: '#fff', borderRadius: 3, padding: '1px 3px' }}>{d}</span> : d}</th>;
                  })}
              ...
              <tbody>
                {visibleStaff.map(s => {
                  ...
                  return (
                    <tr key={s.id}>
                      <td style={{ padding: '5px 10px', position: 'sticky', left: 0, background: '#fff', fontWeight: 500, zIndex: 1, borderBottom: '1px solid #f0efec', whiteSpace: 'nowrap' }}>
                        <div>{s.name.split(' ')[0]}</div><div style={{ fontSize: 9, color: '#a8a79f' }}>{s.branch}</div>
                      </td>
                      {Array.from({ length: dim }, (_, i) => i + 1).map(d => {
                        const isSun = new Date(curY, curM, d).getDay() === 0;
                        ...
                        const bc = val ? attColors[val] : { bg: '#f7f7f5', color: '#d0cfc8' };
                        ...
                        return <td key={d} style={{ padding: '2px', textAlign: 'center', borderBottom: '1px solid #f0efec' }}>
                          {canEdit
                            ? <button onClick={() => setPopup({ staffId: s.id, staffName: s.name, branch: s.branch, dateStr, day: d })} style={{ width: 22, height: 22, borderRadius: 4, border: 'none', cursor: 'pointer', background: bc.bg, color: bc.color, fontSize: 9, fontWeight: 600 }}>{lbl}</button>
                            : <span style={{ width: 22, height: 22, borderRadius: 4, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 600, background: bc.bg, color: bc.color }}>{lbl}</span>}
                        </td>;
                      })}
                      {[p,a,l,h].map((n, i) => {
                        const colors = [{ bg: '#EAF3DE', color: '#27500A' }, { bg: '#FCEBEB', color: '#A32D2D' }, { bg: '#FAEEDA', color: '#633806' }, { bg: '#E6F1FB', color: '#0C447C' }];
                        return <td key={i} style={{ padding: '5px 3px', textAlign: 'center', background: colors[i].bg, color: colors[i].color, fontWeight: 600, borderBottom: '1px solid #f0efec' }}>{n}</td>;
                      })}
                    </tr>
                  );
                })}
      ...
      {tab === 'Monthly' && (
        <div style={{ overflowX: 'auto', border: '1px solid #e3e2dc', borderRadius: 10 }}>
          <table style={{ borderCollapse: 'collapse', fontSize: 12, width: '100%' }}>
            <thead><tr>
              {['Staff','Branch','P','A','L','H'].map(h => <th key={h} style={{ padding: '8px 10px', textAlign: 'left', fontSize: 10, fontWeight: 600, color: '#706f6b', background: '#f7f7f5', borderBottom: '1px solid #e3e2dc', textTransform: 'uppercase' }}>{h}</th>)}
            </tr></thead>
            <tbody>
              {visibleStaff.map(s => {
                let p = 0, a = 0, l = 0, h = 0;
                for (let d = 1; d <= dim; d++) { const v = getAtt(s.id, d); if (v === 'Present') p++; else if (v === 'Absent') a++; else if (v === 'Leave') l++; else if (v === 'Half day') h++; }
                return <tr key={s.id}>
                  <td style={{ padding: '8px 10px', borderBottom: '1px solid #f0efec', fontWeight: 500 }}>{s.name}</td>
                  <td style={{ padding: '8px 10px', borderBottom: '1px solid #f0efec', fontSize: 11 }}>{s.branch}</td>
                  {[p,a,l,h].map((n, i) => {
                    const colors = [{ bg: '#EAF3DE', color: '#27500A' }, { bg: '#FCEBEB', color: '#A32D2D' }, { bg: '#FAEEDA', color: '#633806' }, { bg: '#E6F1FB', color: '#0C447C' }];
                    return <td key={i} style={{ padding: '8px 10px', borderBottom: '1px solid #f0efec', background: colors[i].bg, color: colors[i].color, fontWeight: 600 }}>{n}</td>;
                  })}
                </tr>;
              })}
            </tbody>
      ...
      {popup && (
        <div style={{ position: 'fixed', zIndex: 200, background: '#fff', border: '1px solid #d0cfc8', borderRadius: 10, padding: 10, boxShadow: '0 8px 30px rgba(0,0,0,.12)', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: '#706f6b', marginBottom: 6 }}>{popup.staffName} — {popup.dateStr}</div>
          {[['Present','#EAF3DE','#27500A'],['Absent','#FCEBEB','#A32D2D'],['Leave','#FAEEDA','#633806'],['Half day','#E6F1FB','#0C447C']].map(([s, bg, color]) => (
            <button key={s} onClick={() => mark(popup.staffId, popup.dateStr, s, popup.branch)} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '5px 8px', fontSize: 11, border: '1px solid #e3e2dc', borderRadius: 6, background: bg, color, cursor: 'pointer', marginBottom: 3, fontWeight: 500 }}>{s}</button>
          ))}
...
export function CashFlow() {
  ...
  return (
    <div>
      <SectionHeader left={<div style={{ background: '#f7f7f5', borderRadius: 8, padding: '9px 13px', fontSize: 11, color: '#706f6b' }}>ℹ️ Cash in hand = Yesterday's cash + Today's cash − Today's expenses − Today's deposits</div>} right={<Btn onClick={() => exportCSV('cashflow', [['Date','Branch','Cash','Expenses','Deposits','Cash in hand'], ...filtered.map(r => [r.date,r.branch,r.todayCash,r.todayExp,r.todayDep,r.cashInHand])])}>Export CSV</Btn>} />
      <Stats items={[
        { label: 'Total cash', value: '₹' + fmt(filtered.reduce((s, r) => s + r.todayCash, 0)), color: '#27500A' },
        { label: 'Total expenses', value: '₹' + fmt(filtered.reduce((s, r) => s + r.todayExp, 0)), color: '#A32D2D' },
        { label: 'Total deposits', value: '₹' + fmt(filtered.reduce((s, r) => s + r.todayDep, 0)), color: '#0C447C' },
        { label: 'Cash in hand', value: '₹' + fmt(filtered.length ? filtered[filtered.length - 1].cashInHand : 0), color: '#27500A' },
      ]} />
      <FilterBar>
        <FInput type="date" value={fromDate} onChange={setFromDate} /><FSep />
        <FInput type="date" value={toDate} onChange={setToDate} />
...
export function BankStatement() {
  ...
  const filtered = entries.filter(e => {
    ...
    if (bankF !== 'all' && e.bank !== bankF) return false;
    if (srcF !== 'all' && e.source !== (srcF === 'deposit' ? 'Deposit' : 'Receipt')) return false;
  ...
  let bal = 0;
  const bankColors = { TEW: '#EEEDFE', STEW: '#E1F5EE', Canara: '#FAEEDA', 'ICICI Agro': '#E6F1FB', Agros: '#E6F1FB' };
  ...
  return (
    <div>
      ...
      <FilterBar>
        <FInput type="date" value={fromDate} onChange={setFromDate} /><FSep />
        <FInput type="date" value={toDate} onChange={setToDate} />
        <FSelect value={bankF} onChange={setBankF} options={[{ value: 'all', label: 'All banks' }, 'TEW', 'STEW', 'Canara', 'ICICI Agro']} />
      ...
      <Stats items={[
        ...
        { label: 'From deposits', value: '₹' + fmt(filtered.filter(e => e.source === 'Deposit').reduce((s, e) => s + (e.amount || 0), 0)), color: '#3C3489' },
        { label: 'From receipts', value: '₹' + fmt(filtered.filter(e => e.source === 'Receipt').reduce((s, e) => s + (e.amount || 0), 0)), color: '#633806' },
        { label: 'Total credits', value: '₹' + fmt(filtered.reduce((s, e) => s + (e.amount || 0), 0)), color: '#27500A' },
      ]} />
      <Table cols={['#','Date','Bank','Branch','Source','Reference','Amount','Balance']} rows={filtered.map((e, i) => {
        bal += e.amount || 0;
        return (
          <TR key={i}>
            <TD style={{ color: '#a8a79f' }}>{i+1}</TD><TD>{e.date}</TD>
            <TD><span style={{ background: bankColors[e.bank] || '#F1EFE8', padding: '2px 8px', borderRadius: 20, fontSize: 10, fontWeight: 500 }}>{e.bank}</span></TD>
            <TD>{e.branch}</TD>
            <TD><Badge label={e.source === 'Deposit' ? 'Deposit' : 'Receipt'} /></TD>
            <TD style={{ fontSize: 11, color: '#706f6b' }}>{e.reference}</TD>
            <TD style={{ fontWeight: 500, color: '#27500A' }}>+₹{fmt(e.amount)}</TD>
...
export function SecurityAudit() {
  ...
  if (role !== 'admin') return <div style={{ padding: 40, textAlign: 'center', color: '#706f6b' }}>Admin only</div>;
  return (
    <div style={{ padding: 40, textAlign: 'center', background: '#fff', border: '1px solid #e3e2dc', borderRadius: 8 }}>
      <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 8 }}>Security audit</div>
      <div style={{ fontSize: 13, color: '#706f6b' }}>Firebase Authentication handles all login security.<br />View login activity at <a href="https://console.firebase.google.com" target="_blank" rel="noreferrer" style={{ color: '#534AB7' }}>Firebase Console → Authentication</a></div>
    </div>
  );
}
